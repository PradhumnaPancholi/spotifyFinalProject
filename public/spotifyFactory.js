...
app.factory('spotifyFactory', function ($http, $location) {
  var authToken;
  var retry = 1;
...
//to result.data.access_token (URL, Data, Headers, Authorization all provided by Spotify's Auth. API) & then we are returning that.
  ...
  function trackRequest (token, query){
    ...
    var url = `spotify/v1/search?q=${query}&type=track&limit=50`;
    var headers = {
...
//
  function createSpotifyPlaylist () {
    var tracks = this.savedTracks.map(function(track){
      return track.uri;
    });
    var url = `login?playListName=${this.playListName}&tracks=${tracks}`;
